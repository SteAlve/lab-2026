...
function FilmList({ films, onEdit }) {
    return (
        <ListGroup id="films-list" variant="flush">
            {films.map((film) => <FilmInList filmData={film} key={film.id} onEdit={onEdit} />)}
        </ListGroup>
    );
}
            ...
            <Row className="gy-2">
                <Col xs={6} xl={3} className="favorite-title d-flex gap-2 align-items-center">
                    ...
                    <div className="d-xl-none actions">
                        <i className="bi bi-pencil" style={{ cursor: 'pointer' }} onClick={() => onEdit(filmData)}></i>
                        <i className="bi bi-trash"></i>
                    </div>
                </Col>
                <Col xs={6} xl={3} className="text-end text-xl-center">
                    <span className="custom-control custom-checkbox">
                        <input type="checkbox" className="custom-control-input" defaultChecked={filmData.favorite} readOnly />
